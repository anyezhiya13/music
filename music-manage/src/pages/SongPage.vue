        ...
        <el-dialog title="修改歌曲" :visible.sync="editVisible" width="400px" center>
            <el-form :model="form" ref="form" label-width="80px">
                ...
                <el-form-item prop="introduction" label="专辑" size="mini">
                    <el-input v-model="form.introduction" placeholder="专辑" type="textarea"></el-input>
                </el-form-item>
...
import {songOfSingertId, updateSong, delSong} from '../api/index'
export default {
  mixins: [mixin],
  data () {
    return {
  ...
  methods: {
    ...
    getData () {
      this.tableData = []
      this.tempData = []
      songOfSingertId(this.singerId).then(res => {
        this.tableData = res
        this.tempData = res
        this.currentPage = 1
      })
      ...
      req.onreadystatechange = function () {
        ...
        if (req.readyState === 4 && req.status === 200) {
          ...
          if (res.code) {
            _this.getData()
            _this.registerForm = {}
            _this.notify(res.messsage, 'success')
          } else {
    ...
    // 更新歌曲的url
    uploadSongUrl (id) {
      return `${this.$store.state.HOST}/song/uploadSongUrl?id=${id}`
    },
    // 切换播放歌曲
    setSongUrl (url, name) {
      this.toggle = name
